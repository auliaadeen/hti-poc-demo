    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-neutral-950 px-6 text-neutral-100">
      <div className="mx-auto max-w-2xl text-center">
        <div className="mb-6 inline-flex items-center gap-2 rounded bg-blue-700 px-3 py-1.5">
          <span className="text-sm font-bold text-white">PayrollKita</span>
        </div>
        <h1 className="text-3xl font-bold md:text-4xl">Internal Payroll & HR Tool</h1>
        <p className="mt-3 text-neutral-400">Prototipe MVP — pengganti langganan vendor payroll pihak ketiga.</p>
      </div>
      ...
      <div className="mt-10 grid w-full max-w-2xl grid-cols-1 gap-4 sm:grid-cols-2">
        {menu.map((m) => (
          <Link
            key={m.href}
            href={m.href}
            className="group rounded-xl border border-neutral-800 bg-neutral-900/60 p-6 transition hover:border-blue-700/60 hover:bg-neutral-900"
          >
            <h2 className="text-lg font-semibold text-white group-hover:text-blue-100">{m.title}</h2>
            <p className="mt-1.5 text-sm text-neutral-400">{m.desc}</p>
          </Link>
        ))}
      </div>
